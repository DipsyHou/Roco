...
- （强化模块：状态效果。拥有自身提供的护盾的目标造成的伤害提高16%。）
- （急速模块：状态效果。拥有自身提供的护盾的目标速度提高12%。）
- （抵御模块：状态效果。拥有自身提供的护盾的目标被施加负面效果时，有50%概率避免此次负面效果。）
...
**王权光环**（被动）：对敌方目标发动普通攻击或技能时，自身比目标中速度最大者每高出30点速度，增加一段此次普攻或技能的连击数，最多增加3段连击；然后使所有目标速度降低10%，持续1回合。
