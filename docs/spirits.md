...
**紧急支援**（被动）：某个己方精灵生命值比例低于30%时，立刻为其回复（100%自身魔攻）点血量，净化其所有负面效果并提升其25%速度（1回合）。每局对战最多触发一次。
...
**棘皮**（3）：目标为全体己方精灵。给予所有目标“棘皮”正面效果：受到伤害后，移除此效果并获得施加者给予的（30%施加者物防）护盾，持续2回合，以这种方式再次获得护盾时可累积护盾量并刷新护盾持续时间，上限为（60%施加者物防）。若目标已拥有“棘皮”效果则不再给予。
...
**防火墙**（4）：目标为全体己方精灵。赋予所有目标（90%自身物攻）护盾，持续3回合，持续期间再次获得自身赋予的护盾会累积护盾量并刷新持续时间，上限为（120%施加者物攻）。
...
**养分输送**（被动）：队友受到伤害时，若自身生命值比例高于50%，则为该队友回复（2%自身最大生命值）点生命值；否则为自己回复（2%自身最大生命值）点生命值。

**普通攻击**：目标为一个敌方精灵。对目标造成（100%自身物攻）点物理伤害。

**深根**（4）：目标为自身。获得“深根”，持续3回合，期间再次使用此技能会重置“深根”持续时间。

- （深根：状态效果。速度降低40%，队友受到伤害时自身分摊50%的伤害量。）

**寄生种子**（1）：目标为一个敌方精灵。赋予目标4层“寄生”效果。

- （寄生：负面效果，按施加者分别叠加。携带者回合结束时，每个来源各结算一次（4%×施加者魔攻×该来源层数）魔法伤害，仅吃持续伤害专用增减伤；施加者为自身回复等同于该次伤害段（扣盾前）100%的血量；然后该来源层数减少1。）

**紧缠**（3）：目标为全体敌方精灵。赋予全体目标2层“寄生”效果，然后触发所有目标的“寄生”效果。
