...
**紧急支援**（被动）：某个己方精灵生命值比例低于30%时，立刻为其回复（100%自身魔攻）点生命，净化其所有负面效果并提升其25%速度（1回合）。每局对战最多触发一次。

**普通攻击**：目标为一个敌方精灵。对目标造成（100%自身物攻）点物理伤害。

**光合作用**（2）：目标为一个己方精灵。为目标回复（80%自身魔攻）点生命；若目标是自身，则额外解除随机一个负面效果。
...
**共振**（被动）：自身技能固定不消耗能量，秘能上限为8点。对局开始时获得4点秘能。队友对敌方目标发动攻击时，若自身拥有不少于1点秘能，则消耗1点秘能对随机一个目标造成40固定附加伤害。
...
**灵珏御光**（被动）：灵珏护体，受到每段伤害时，降低（10%自身灵气层数，向下取整）点伤害，并赋予自身（实际减少的伤害值）层灵气。自身死亡时，若尚无“通灵”且有灵气层数，则将生命回复至（自身灵气层数）点，并获得“通灵”。
...
**缓冲**（被动）：若我方精灵在一回合内消耗了不少于5点能量，回合结束时回复1点能量。
...
**过载**（5）：目标为一个敌方精灵。对目标造成（200%自身魔攻）点魔法伤害，然后使目标与自身的速度降低20%，持续2回合。
...
- （彻甲：状态效果。对敌方精灵释放普攻或技能时，若自身拥有“罡气”且目标“驱邪”层数小于4层，则移除自身1层“罡气”，并赋予目标1层“驱邪”。对拥有2/3/4层“驱邪”的目标造成伤害时，此伤害暴击率提升25%/35%/60%。）
...
| 能力值 | 生命 | 物攻 | 魔攻 | 物防 | 魔防 | 速度 |
| --- | --- | --- | --- | --- | --- | --- |
| | 650 | 100 | 130 | 120 | 120 | 120 |

**收藏灵魂**（被动）：自身技能固定不消耗能量。对局开始时获得2点秘能，秘能上限为20点。自身成为己方精灵技能的唯一目标时，回复2点秘能。当自身秘能从小于13点增长至大于等于13点时，使自身下回合提前100%，并使自身的“恐怖”效果延长1回合。
...
**敏锐**（被动）：发动普通攻击时，若目标拥有不少于3条“破绽”，则使此次攻击伤害提高25%，并获得一次额外行动。
...
- 水煮鱼：目标为自身。赋予目标“水煮鱼”正面效果，持续3回合：基于携带者物攻的10%，提升所有队友的物攻与魔攻；持续期间再次获得此效果会重置持续时间。
...
**血墙**：此技能固定不消耗能量。目标为自身。消耗10%最大生命值（不致死），提高自身50%双防，持续4回合，期间再次获得此效果会重置持续时间。

**狂宴**（3）：目标为自身。回复（33%自身已损生命）生命。

**临行留念**：此技能固定不消耗能量。目标为全体敌方精灵。对所有目标造成（50%自身魔攻+10%自身已损生命）魔法伤害；使自身死亡。


## 石化刺蜥蜴

![](../assets/spirits/石化刺蜥蜴.png)

| 能力值 | 生命 | 物攻 | 魔攻 | 物防 | 魔防 | 速度 |
| --- | --- | --- | --- | --- | --- | --- |
| | 550 | 100 | 100 | 150 | 150 | 120 |

**硬化肌肤**（被动）：受到的固定伤害降低50%。

**普通攻击**：目标为一个敌方精灵。对目标造成（100%自身物攻）物理伤害。

**棘皮**（3）：目标为全体己方精灵。给予所有目标“棘皮”正面效果：受到伤害后，移除此效果并获得施加者给予的（30%施加者物防）护盾，持续3回合，以这种方式再次获得护盾时可累积护盾量并刷新护盾持续时间，上限为（60%施加者物防）。若目标已拥有“棘皮”效果则不再给予。

**再生**（被动）：自身受到伤害后，提升自身20%双防，并减少“棘皮”技能2点能耗，持续1回合，期间再次获得此效果会延长1回合持续时间，最多持续2回合。

**岩刺**（1）：目标为一个敌方精灵。对自身造成（20%自身物攻）物理伤害，然后对目标造成（120%自身物防 + 100%自身护盾量）物理伤害。


## 机械方方

![](../assets/spirits/机械方方.png)

| 能力值 | 生命 | 物攻 | 魔攻 | 物防 | 魔防 | 速度 |
| --- | --- | --- | --- | --- | --- | --- |
| | 650 | 130 | 100 | 130 | 100 | 130 |

**多色模块**（被动）：游戏开始时获得“强化模块”效果。使用技能后，若自身拥有“强化模块”/“急速模块”/“抵御模块”效果，则移除此效果，并获得“急速模块”/“抵御模块”/“强化模块”效果。
- （强化模块：状态效果。拥有自身提供的护盾的目标造成的伤害提高12%。）
- （急速模块：状态效果。拥有自身提供的护盾的目标速度提高6%。）
- （抵御模块：状态效果。拥有自身提供的护盾的目标受到的伤害降低12%。）

**普通攻击**：目标为一个敌方精灵。对目标造成（100%自身物攻）物理伤害。

**防火墙**（3）：目标为全体己方精灵。赋予所有目标（60%自身物攻）护盾，持续3回合，持续期间再次获得自身赋予的护盾会累积护盾量并刷新持续时间，上限为（120%施加者物攻）。

**便携防火墙**（0）：目标为一个己方精灵。赋予目标（30%自身物攻）护盾，持续3回合，持续期间再次获得自身赋予的护盾会累积护盾量并刷新持续时间，上限为（120%施加者物攻）。

**超限模块**（3）：目标为自身。获得“超限模块”状态效果，持续4回合。拥有“超限模块”时，同时获得“强化模块”，“急速模块”与“抵御模块”的效果。
...
| 能力值 | 生命 | 物攻 | 魔攻 | 物防 | 魔防 | 速度 |
| --- | --- | --- | --- | --- | --- | --- |
| | 800 | 100 | 120 | 110 | 110 | 100 |


**养分输送**（被动）：我方精灵受到伤害后，若自身生命值比例高于50%，则为该精灵回复（2%自身最大生命）生命；否则为自己回复（2%自身最大生命）生命。

**普通攻击**：目标为一个敌方精灵。对目标造成（100%自身物攻）点物理伤害。

**深根**（4）：目标为自身。获得“深根”，持续3回合，期间再次使用此技能会重置“深根”持续时间。

- （深根：状态效果。速度降低50%，队友受到伤害时自身分摊40%的伤害量。）

**寄生种子**（1）：目标为一个敌方精灵。赋予目标4层“寄生”效果。

- （寄生：负面效果，可叠加。携带者回合结束时，受到一次（4%×施加者魔攻×层数）魔法伤害，施加者为自身回复等同于此次伤害值100%的生命；然后层数减少1。）

**紧缠**（2）：目标为全体敌方精灵。赋予全体目标2层“寄生”效果，然后触发所有目标的“寄生”效果。



## 棋绮后

![](../assets/spirits/棋绮后.png)

**荣勋**（被动）：自身在场时，队友可以通过以下方式被自身赋予“军功”效果：攻击敌方精灵时，获得1层；击败敌方精灵时，获得4层。“军功”最多叠加8层。

**战场晋升**（被动）：自身在场时，队友“军功”叠加至8层后，赋予其“爵位”效果。
...
**王权光环**（被动）：对敌方目标发动普通攻击或技能时，自身比目标中速度能力值最高者每高出30点速度，增加一段此次普攻或技能的连击数，最多增加3段连击。

**普通攻击**：目标为一个敌方精灵。2连击：每次对目标造成（30%自身物攻）点物理伤害。

**威压**（4）：目标为全体敌方精灵。4连击：每次对随机一个目标造成（30%自身物攻）点物理伤害，并使其速度降低10%，持续1回合。

**振羽**（0）：目标为自身。使目标获得“振羽”状态效果：发动攻击时，使此次攻击倍率提高（20%自身物攻），然后移除此效果。

**驭风**（3）：目标为一个敌方精灵。2连击：每次对目标造成（30%自身物攻）点物理伤害，提升自身10%速度，持续3回合。
