...
**王权光环**（被动）：对敌方目标发动普通攻击或技能时，自身比目标中速度能力值最高者每高出30点速度，增加一段此次普攻或技能的连击数，最多增加3段连击；然后使所有目标速度降低10%，持续1回合。

**普通攻击**：目标为一个敌方精灵。2连击：每次对目标造成（30%自身物攻）点物理伤害。

**威压**（4）：目标为全体敌方精灵。5连击：每次对随机一个目标造成（30%自身物攻）点物理伤害，并使其速度降低10%，持续1回合。

**振羽**（0）：目标为自身。使目标获得“振羽”状态效果：发动攻击时，使此次攻击倍率提高（20%自身物攻），然后移除此效果。

**驭风**（3）：目标为一个敌方精灵。3连击：每次对目标造成（30%自身物攻）点物理伤害，提升自身10%速度，持续3回合。